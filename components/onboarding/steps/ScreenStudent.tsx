...
export default function ScreenStudent({ answers, updateAnswers, onNext, onBack, canGoBack, isLoading }: Props) {
  const config = STEP_CONFIGS.ScreenStudent;

  const handleAnswer = (student: boolean) => {
    ...
    <div className="text-center">
      ...
      <div className="space-y-4">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-gray-900">はい、学生です</p>
              <p className="text-sm text-gray-600">大学・専門学校・高校に在学中</p>
            </div>
          </div>
        </button>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-gray-900">いいえ、学生ではありません</p>
              <p className="text-sm text-gray-600">社会人・フリーターなど</p>
            </div>
          </div>
        </button>
      </div>
      ...
      <div className="mt-4 p-3 bg-blue-50 rounded-lg">
        <p className="text-sm text-blue-800">
          💡 学生の場合、特定扶養控除（150万円）が適用される可能性があります
        </p>
      </div>
    </div>
  );
